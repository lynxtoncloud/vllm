...
单卡SDPA通过并不能验证整个视觉编码器或TP16通信。需要定位时，在一份
独立配置的environment中设置 `VLLM_GLM5NEXT_TRACE_VISION=1`，保留
`ENFORCE_EAGER=1`，并使用新RUN_ID。不要开启视觉编译或视觉CUDA graph。
管理器会清除终端继承的这个开关，仅采用配置文件显式指定的值。

每个视觉模块打印 `GLM vision trace`，带rank、PID、时间及模块名：

- `BEGIN`：即将等待前序GPU工作完成。
- `CALL`：前序工作已完成，即将调用本模块。
- `RETURN`：模块已返回，正在等待本模块提交的GPU工作。
- `DONE`：本模块GPU工作已完成。

视觉调用期间每60秒打印全部Python线程栈，调用结束或抛异常后取消定时器。
比较16个rank最后的标记和线程栈，区分输入等待、算子调用、GPU同步与
模块内部的TP通信。全部视觉模块DONE但服务仍停滞时，再检查下游语言模型。
模块名包含通信的线性层时，单靠模块名不能判定是GEMM还是通信失败。

这个开关会改变同步时序，可能使竞态不再出现；诊断成功不代表原问题修复。
不更改权重、精度、分辨率或超时，默认关闭且仅作用于GLM视觉塔。
诊断日志不能用于正式性能报告；恢复全量测评前关闭开关并新建RUN_ID。
