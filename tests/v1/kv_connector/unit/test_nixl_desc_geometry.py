...
def test_overlaid_transfer_groups_share_region_geometry(padded_tail):
    ...
    num_blocks = 4
    spec: KVCacheSpec = MLAAttentionSpec(
        block_size=4,
        num_kv_heads=1,
        head_size=8,
        dtype=torch.uint8,
    ...
    if padded_tail:
        num_blocks, page_size, block_stride, payload_size = 3976, 12672, 12672, 2048
        spec = KpoolTailSpec(
            block_size=4,
            num_kv_heads=2,
            head_size=128,
...
def test_kpool_tail_transfer_preserves_padding_and_other_blocks(remote_stride):
    """Transfer live payloads across unequal P/D pitches without touching padding."""
    from types import SimpleNamespace

    from vllm.distributed.kv_transfer.kv_connector.v1.nixl import base_worker as bw
    from vllm.v1.kv_cache_interface import KpoolTailSpec

    count, local_stride, payload = 3976, 12672, 2048
