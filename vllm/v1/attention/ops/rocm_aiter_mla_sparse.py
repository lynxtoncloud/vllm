...
def fp8_paged_mqa_logits_torch(
    ...
    cache_flat = kv_cache.view(-1, block_size * (dim + 4))
    for i in range(batch_size):
        if context_lens.ndim == 1:
            # A scalar context length describes the final speculative row.
            final_len = int(context_lens[i].item())
            lengths = list(range(final_len - next_n + 1, final_len + 1))
        else:
            lengths = context_lens[i].tolist()
        seq_len = max(lengths)
        assert 0 <= seq_len <= max_model_len
        if seq_len == 0:
            continue
        num_pages = cdiv(seq_len, block_size)
        pages = cache_flat[block_tables[i, :num_pages]]
        scale_offset = block_size * dim
        values = pages[:, :scale_offset].view(fp8_dtype)
        # AMD stores 16x16 value tiles followed by all scales in each page.
        # The tensor's nominal [page, token, 1, D+4] shape is not its layout.
        if block_size > 1:
            values = (
                values.reshape(num_pages, block_size // 16, dim // 16, 16, 16)
                .transpose(2, 3)
                .reshape(num_pages * block_size, dim)
            )
        else:
            values = values.reshape(num_pages, dim)
        values = values[:seq_len].float()
        scales = pages[:, scale_offset:].contiguous().view(torch.float32).flatten()
        for j, length in enumerate(lengths):
            if length <= 0:
                continue
            row = i * next_n + j
            score = F.linear(values[:length], q[i, j].float())
            score.relu_()
            score *= weights[row][None, :]
            logits[row, :length] = score.sum(dim=1) * scales[:length]
    return logits
...
_TORCH_MQA_LOGITS_MAX_SCORE_BYTES = 64 * 1024 * 1024
...
def fp8_mqa_logits_torch(
    ...
    # Bound the [H, rows, N] intermediate independently of the logits budget.
    # The indexer profiles [M, N], not a full copy for every attention head.
    rows_per_chunk = max(
        1, _TORCH_MQA_LOGITS_MAX_SCORE_BYTES // (4 * max(q.shape[1] * seq_len_kv, 1))
    )
    logits = torch.empty((q.shape[0], seq_len_kv), device=device, dtype=torch.float32)
    columns = torch.arange(seq_len_kv, device=device)[None, :]
    for start in range(0, q.shape[0], rows_per_chunk):
        end = min(start + rows_per_chunk, q.shape[0])
        score = torch.einsum("mhd,nd->hmn", q[start:end], k).float()
        score.mul_(scale.reshape(-1)).relu_()
        score.mul_(weights[start:end].T.unsqueeze(-1))
        chunk = score.sum(dim=0)
        mask = (columns >= cu_seqlen_ks[start:end, None]) & (
            columns < cu_seqlen_ke[start:end, None]
        )
        logits[start:end] = chunk.masked_fill_(~mask, float("-inf"))
